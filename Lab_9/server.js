...
app.post("/add", (request, response) => {
  if(typeof request.body.num1 != "number" || typeof request.body.num2 != "number")
    response.status(400).send("Both num1 and num2 must be defined.");
  else
    response.status(200).send({value: request.body.num1 + request.body.num2});
});

app.post("/divide", (request, response) => {
  if(typeof request.body.num1 != "number" || typeof request.body.num2 != "number")
    response.status(400).send("Both num1 and num2 must be defined.");
  else if(request.body.num2 == 0)
    response.status(400).send("Cannae divide by zero!");
  else
    response.status(200).send({value: request.body.num1 / request.body.num2});
});
