...
function begin_play(){
	...
	if(p1.value != "" && p2.value != ""){
		...
		for(var i = 0; i < table_ids.length; i++)
			table_elms[i] = document.getElementById(table_ids[i]);
	}else{
		error.style.top = 0;
		error.innerHTML = "<b>Error:</b> Both names must be filled before playing the game.";
	}
}
...
function reset_play(){
	...
	p1.value = "";
	p2.value = "";
	ti.innerHTML = "Game has not begun.";
	board_state = [-1,-1,-1,-1,-1,-1,-1,-1,-1];
	for(var i = 0; i < table_ids.length; i++)
		table_elms[i].innerHTML = table_ids[i];
	document.getElementById("bgn").disabled = false;
...
function checkWin(){
	...
	for(var r = 0; r < 9; r += 3)
		if(board[r] != -1 && board[r] == board[r + 1] && board[r + 1] == board[r + 2])
			return board[r];
	for(var c = 0; c < 3; c++)
		if(board[c] != -1 && board[c] == board[c + 3] && board[c + 3] == board[c + 6])
			return board[c];
...
function play() {
	var ch = document.getElementById("move_text_id");
	ch.value = ch.value.toUpperCase();
	if(table_ids.includes(ch.value) && board_state[table_ids.indexOf(ch.value)] == -1){
		error.style.top = "-60px";
		board_state[table_ids.indexOf(ch.value)] = turn;
		table_elms[table_ids.indexOf(ch.value)].innerHTML = turn ? "X" : "O";
		ch.value = "";
		var win = checkWin();
		if(win == -1){
			toggle_move();
			ti.innerHTML = "It is " + (turn ? p1 : p2) + "'s turn.";
		}else{
			// ti.innerHTML = "Game over, " + (turn ? p1 : p2) + " won the game!";
			alert((turn ? "X" : "O") + " won the game!");
			reset_play();
		}
	}else{
		error.style.top = 0;
		error.innerHTML = "<b>Error:</b> Please enter a valid, unoccupied position.";
	}
}
